...
function brandCount = countBrandOnes(datasetPath)
global numSamples
filename = sprintf('%s/BRAND_%d.mat', datasetPath, numSamples);
BRAND = load(filename);
brand_sc2 = BRAND.referBrand;
filename = sprintf('%s/BRAND_Scale%d.mat', datasetPath, 1);
BRAND = load(filename);
brand_sc1 = BRAND.queryBrand;
filename = sprintf('%s/BRAND_Scale%d.mat', datasetPath, 4);
BRAND = load(filename);
brand_sc4 = BRAND.queryBrand;

brandCount = [mean(sum(brand_sc1,2)) mean(sum(brand_sc2,2)) mean(sum(brand_sc4,2))]
end
