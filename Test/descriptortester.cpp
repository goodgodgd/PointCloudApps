...
DescriptorTester::DescriptorTester()
{
}

void DescriptorTester::TestDescriptor()
{
    ...
    CreatePointCloud(trueDesc, neighborCloud);
    TransformPointCloud(rotation, translation, neighborCloud, NUM_PTS, normalvt);
    descriptor = ComputeEachDescriptor(translation, normalvt, neighborCloud, neibIndices, 0, NUM_PTS);

    qDebug() << "TestDescriptor true descriptor:" << trueDesc << "computed descriptor" << descriptor;
}

int DescriptorTester::CreatePointCloud(cl_float4 trueDesc, cl_float4* pointCloud)
{
...
void DescriptorTester::TransformPointCloud(cl_float4 rotation, cl_float4 translation, cl_float4* pointCloud, int num_pts, cl_float4& normalvt)
{
    cl_float4 rotmat[3];
